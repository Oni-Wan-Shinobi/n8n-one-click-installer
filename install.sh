...
read -rp "  [1/7] IP этого сервера [${DETECTED_IP}] — нажмите Enter чтобы оставить: " SERVER_IP
...
read -rp "  [7/7] Timezone [${DETECTED_TZ}] — нажмите Enter чтобы оставить: " USER_TZ
...
echo ""
read -rp "Продолжить? [Y/n] — нажмите Enter чтобы продолжить: " CONFIRM
CONFIRM="${CONFIRM:-Y}"
if [[ "$CONFIRM" =~ ^[Nn]$ ]]; then
  echo "Отменено."
  exit 0
